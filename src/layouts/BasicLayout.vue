  ...
  <div class="BasicLayout">
    <a-layout class="ant-layout-has-sider">
      ...
      <a-layout>
        ...
        <context-menu ref="contextMenuRef" :itemList="state.menuItemList" v-model:visible="state.menuVisible" :x="state.menuX" :y="state.menuY" @select="onMenuSelect"/>
        <a-tabs hideAdd v-model:activeKey="state.activeKey" @contextmenu="e => onContextmenu(e)" type="editable-card"
          ...
          <a-tab-pane v-for="pane in state.panels" :key="pane.key" :tab="pane.title" :closable="pane.closable" :data-pagekey="pane.key">
...
import { computed, markRaw, onUpdated, reactive, ref, shallowRef, watch, onMounted, defineAsyncComponent } from 'vue'
...
const Main = defineAsyncComponent(() => import('@/views/index/Main.vue'))
...
let contextMenuTimeout = null; // 防抖定时器
const state = reactive({
  ...
  nextKey: '',
  menuItemList: [
    { key: '1', text: '关闭左边', icon: 'LeftOutlined' },
    { key: '2', text: '关闭右边', icon: 'RightOutlined' },
    { key: '3', text: '关闭其他', icon: 'CloseOutlined' }
  ]
})
...
function initSpringDocOpenApi() {
  ...
  localStore.getItem(constant.globalSettingsKey).then(settingCache => {
    ...
    //console.log("layout---")
    localStore.getItem(constant.globalGitApiVersionCaches).then(cacheApis => {
...
function onMenuSelect(key, target) {
  console.log("onMenuSelect triggered with key:", key, "target:", target);
  
  // 提取真正的key值
  const actualKey = typeof key === 'object' && key.key ? key.key : key;
  
  // 使用右键点击时保存的contextTabKey
  const pageKey = state.contextTabKey || state.activeKey;
  
  console.log("Executing menu action:", actualKey, "for pageKey:", pageKey);
  ...
  switch (actualKey) {
    ...
    case "3":
      ...
      break;
    default:
      console.log("Unknown menu key:", actualKey);
      break;
  }
}

function onContextmenu(e) {
  console.log("onContextmenu triggered, target:", e.target);
  
  // 检查是否点击在标签按钮上
  const tabButton = e.target.closest('.ant-tabs-tab-btn');
  if (!tabButton) {
    console.log("Not clicking on a tab button");
    return;
  }
  
  const pageKey = getPageKey(e.target);
  console.log("pageKey found:", pageKey);
  
  if (pageKey !== null) {
    e.preventDefault();
    e.stopPropagation();
    console.log("Setting menu position and visibility", { x: e.clientX, y: e.clientY });
    
    // 设置菜单位置和右键点击的标签键
    state.menuX = e.clientX;
    state.menuY = e.clientY;
    state.contextTabKey = pageKey;
    state.menuVisible = true;
  } else {
    console.log("No pageKey found, not showing menu");
  }
}

function getPageKey(target) {
  // 简化版本，基于原始knife4j实现，适配Ant Design Vue结构
  console.log("getPageKey called with target:", target);
  
  // 首先尝试从当前元素及其父元素查找pagekey
  let element = target;
  while (element && element !== document) {
    const pagekey = element.getAttribute("pagekey") || element.getAttribute("data-pagekey");
    console.log("Checking element:", element, "pagekey:", pagekey);
    if (pagekey) {
  ...
  // 如果没找到，尝试通过aria-controls属性推断
  const tabButton = target.closest('.ant-tabs-tab-btn');
  if (tabButton) {
    const ariaControls = tabButton.getAttribute('aria-controls');
    console.log("Found tab button with aria-controls:", ariaControls);
    if (ariaControls) {
      // 从aria-controls中提取tab的key
      // 格式通常是: rc-tabs-0-panel-{key}
      const match = ariaControls.match(/panel-(.+)$/);
      if (match) {
        const extractedKey = match[1];
        console.log("Extracted key from aria-controls:", extractedKey);
        return extractedKey;
      }
    }
  }
  
  console.log("No pagekey found");
  return null;
}

function closeOthers(pageKey) {
  console.log("closeOthers called with pageKey:", pageKey);
  console.log("Current linkList:", state.linkList);
  console.log("Current panels:", state.panels.map(p => p.key));
  ...
  console.log("New linkList:", state.linkList);
  console.log("New panels:", state.panels.map(p => p.key));
}

function closeLeft(pageKey) {
  console.log("closeLeft called with pageKey:", pageKey);
  console.log("Current linkList:", state.linkList);
  console.log("Current panels:", state.panels.map(p => p.key));
  ...
  if (state.linkList.length > 2) {
    ...
    console.log("pageKey index:", index);
    
    if (index === -1) {
      console.log("pageKey not found in linkList!");
      return;
    }
    ...
    console.log("Not enough tabs to close left (need > 2, have:", state.linkList.length, ")");
  }
}

function closeRight(pageKey) {
  console.log("closeRight called with pageKey:", pageKey);
  console.log("Current linkList:", state.linkList);
  console.log("Current panels:", state.panels.map(p => p.key));
  ...
  console.log("New linkList:", tmpLinks);
  console.log("New panels:", tmpTabs.map(p => p.key));
