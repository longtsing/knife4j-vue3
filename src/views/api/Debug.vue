...
export default {
  ...
  methods: {
    ...
    setResponseRaw(res) {
      console.log("setResponseRaw called with:", res);
      if (KUtils.checkUndefined(res)) {
        var resp = res.request || res;
        ...
        if (KUtils.checkUndefined(resp)) {
          console.log("responseType:", resp.responseType);
          
          // 强制获取Raw文本，不管responseType是什么
          var _tmpRawText = "";
          
          // 对于blob类型，需要特殊处理
          if (resp.responseType === "blob" && res.data instanceof Blob) {
            console.log("Handling blob response, size:", res.data.size);
            // 对于blob类型，尝试将其转换为文本
            var reader = new FileReader();
            reader.onload = () => {
              _tmpRawText = reader.result;
              console.log("Blob converted to text, length:", _tmpRawText ? _tmpRawText.length : 0);
              this.responseRawText = _tmpRawText;
              console.log("responseRawText set from blob:", this.responseRawText.length + " chars");
            };
          ...
          if (resp.responseText) {
            _tmpRawText = KUtils.toString(resp.responseText, "");
            console.log("Got text from resp.responseText");
          } else if (resp.response && typeof resp.response === 'string') {
            _tmpRawText = KUtils.toString(resp.response, "");
            console.log("Got text from resp.response");
          } else if (res.data) {
            if (typeof res.data === 'string') {
              _tmpRawText = res.data;
              console.log("Got text from res.data (string)");
            } else if (typeof res.data === 'object') {
              _tmpRawText = JSON.stringify(res.data, null, 2);
              console.log("Got text from res.data (JSON)");
            }
          }
          
          console.log("Raw text length:", _tmpRawText ? _tmpRawText.length : 0, "chars");
          console.log("Raw text preview:", _tmpRawText ? _tmpRawText.substring(0, 200) + "..." : "empty");
          
          if (_tmpRawText) {
            this.responseRawText = _tmpRawText;
            console.log("responseRawText set to:", this.responseRawText.length + " chars");
          } else {
            console.log("No raw text found in response");
            // 如果找不到文本，尝试从其他地方获取
            if (res && res.config && res.config.url) {
              this.responseRawText = "无法获取原始响应文本。请求URL: " + res.config.url;
            } else {
              this.responseRawText = "无法获取原始响应文本";
            }
          }
        } else {
    ...
    setResponseCurl(resp) {
      console.log("setResponseCurl called with:", resp);
      var that = this;
      var url = this.debugCheckUrl(this.debugUrl);
      console.log("setResponseCurl url:", url);
      // 构建请求响应CURL
      ...
      var fullurl = "";
      // 使用实际发出的请求URL来生成curl，避免context-path重复问题
      ...
      console.log("Generated curl command:", this.responseCurlText);
    },
    debugFormCurlParams() {
      var params = {};
      this.formData.forEach(form => {
    ...
    setResponseJsonBody(resp, headers) {
      ...
      if (mode == "json") {
        ...
        var maxSize = 150; // 恢复150KB限制
        // 数据大小
        this.bigBlobFlag = mbSize > 300; // 300KB以上显示大数据标志
        // console.log(mbSize)
        if (mbSize > maxSize) {
          this.bigFlag = true;
          // 超出限制时，格式化显示中不显示内容，但Raw中仍可查看
          _text = `数据过大 (${mbSize}KB)，请在Raw标签页中查看完整内容\n\n原始数据类型: ${mode.toUpperCase()}\n数据大小: ${mbSize}KB\n\n提示: 点击上方"Raw"标签页可查看完整的原始响应数据`;
          var messageInfo = this.i18n.message.debug.contentToBig + ` (${mbSize}KB)，请在Raw中查看`;
