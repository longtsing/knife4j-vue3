...
function SwaggerBootstrapUi(options) {
  ...
  //  LiteStar 集成标识
  this.framework = options.framework || 'LiteStar';
  this.frameworkVersion = options.frameworkVersion || '2.17.0';
  //  禁用 basePath 自动处理
  this.disableBasePath = options.disableBasePath || false;
...
SwaggerBootstrapUi.prototype.createApiInfoInstance = function (path, mtype, apiInfo, appendBaePathFlag) {
  ...
  // LiteStar 特殊处理：检查是否是 LiteStar 环境并且禁用 basePath
  if (that.springdoc && !that.disableBasePath) {
