...
//
// 设计上不在 handlers 内部塞反射（gin router 的 handler 是函数，无法稳定反射出
// 入参/出参），而采用"路由注册处旁路调用 Register(...)"的方式集中维护。
// 比手工维护 openapi.json 文件更安全（一次只改一处，避免漏标）。
package docs
...
func build() {
	spec = &Spec{
		...
		Info: Info{
			...
			Version:     "1.0.0",
			Description: "Knife4j Vue3 + Go Gin 示例项目，展示如何集成 API 文档界面。",
		},
		Servers: []Server{{URL: "/"}},
		Tags: []Tag{
